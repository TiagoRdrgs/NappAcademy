...
class TestPedido:
    ...
    def test_nota_fiscal(self):
        cliente = Cliente(nome='José da Silva')
        pedido = Pedido(cliente)
        pedido.add_item(Produto(descricao='Suco', ean='123', preco=10))
        pedido.add_item(Produto(descricao='Doce', ean='123546', preco=5))
        pedido.add_item(Produto(descricao='Suco', ean='123', preco=10))
    ...
    def test_checkout_fail2(self):
        ...
        with pytest.raises(Exception) as error:
            pedido = Pedido(Cliente(nome='José da Silva'))
            pedido.add_item(Produto(ean='123', preco=10))
            pedido.checkout()
        assert str(error.value) == msg_erro
